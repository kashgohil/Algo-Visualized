...
const selectionSort = (randomArray) => {
	...
	for (let i = 0; i < len - 1; i++) {
		...
		[copy[i], copy[minInd]] = [copy[minInd], copy[i]];
	}
	return ans;
};
...
const heapSort = (randomArray) => {
	const heapify = (copy, len, i, ans) => {
		...
		if (left < len && copy[left] > copy[largest]) largest = left;

		ans.push({ first: right, second: largest, action: 'comp' });
		if (right < len && copy[right] > copy[largest]) largest = right;
	...
	for (let i = len / 2 - 1; i >= 0; i--) heapify(copy, len, i, ans);
